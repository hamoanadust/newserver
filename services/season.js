...
const find_season = async data => {
    try {
        ...
        return success_res(resp[0], resp[0] ? undefined : 'season not found');
    } catch (err) {
        console.log('find season err');
        throw(err);
    }
}
...
const prepare_season_invoice_data = async data => {
    let { carpark_id, card_number, start_date, end_date, //required
        card_type, vehicle_type, holder_type, invoice_type, attn, invoice_number, invoice_date, vehicle_id, //optional wtth a default value
    ...
    //validation
    if (!carpark_id) return fail_res('no carpark id');
    else if (!card_number) return fail_res('no card number');
    else if (!start_date) return fail_res('no start date');
    else if (!end_date) return fail_res('no end date');
    else if (moment(start_date).isAfter(moment(end_date))) return fail_res('start date is after end date');
    //set default value
    ...
    //validation again
    if (!carpark || carpark.length === 0) return fail_res('no carpark');
    else if (!season_rate || season_rate.length === 0) return fail_res('no season rate');
    else if (!system_config) return fail_res('no system config');
    ...
    const invoice_data = {invoice_number, invoice_date, invoice_type, invoice_amount, attn, carpark_id, carpark_name, carpark_address, 
    ...
    return new Promise((resolve, reject) => {
        db.pool.getConnection((conn_err, connection) => {
            ...
            connection.beginTransaction(async trans_err => {
                ...
                try {
                    //create season with season_data
                    connection.query('insert into season set ?', season_data, async (season_err, season_created, fields) => {
...
const add_season = async data => {
    try {
        ...
        return success_res(season_data);
    } catch (err) {
        console.log('add_season err');
        throw(err);
    }
}
