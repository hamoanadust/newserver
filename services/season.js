...
const list_all_season = async data => {
    try {
        ...
        const sql = `select s.*, c.carpark_name, c.carpark_code, c.address, c.postal_code, c.public_policy, c.billing_method, c.allow_giro, c.allow_auto_renew, c.status as carpark_status, c.remarks, c.giro_form_id, item.invoice_item_id, item.unit_price, item.quantity, item.amount, item.invoice_id, item.description, item.*, f.file_type, f.name as filename, f.mimetype, f.size, sr.season_rate_id, sr.rate, sr.status as rate_status, sr.remarks as rate_remarks from season s left join carpark c using(carpark_id) left join invoice_item item on item.invoice_item_id = (select invoice_item.invoice_item_id from invoice_item where s.season_id = invoice_item.season_id order by invoice_item.invoice_item_id desc limit 1) left join file f using (file_id) left join season_rate sr on (sr.carpark_id = c.carpark_id && sr.vehicle_type = s.vehicle_type && sr.season_type = s.season_type && sr.status = 'ACTIVE') where s.holder_id = ${db.escape(holder_id)} and s.status in ('NEW', 'ACTIVE') and s.is_latest = true`
...
const list_season = async data => {
    try {
        ...
        const sql = `select s.*, c.carpark_name, c.carpark_code, c.address, c.postal_code, c.public_policy, c.billing_method, c.allow_giro, c.allow_auto_renew, c.status as carpark_status, c.remarks, c.giro_form_id, item.invoice_item_id, item.unit_price, item.quantity, item.amount, item.invoice_id, item.description, item.*, f.file_type, f.name as filename, f.mimetype, f.size, sr.season_rate_id, sr.rate, sr.status as rate_status, sr.remarks as rate_remarks from season s left join carpark c using(carpark_id) left join invoice_item item on item.invoice_item_id = (select invoice_item.invoice_item_id from invoice_item where s.season_id = invoice_item.season_id order by invoice_item.invoice_item_id desc limit 1) left join file f using (file_id) left join season_rate sr on (sr.carpark_id = c.carpark_id && sr.vehicle_type = s.vehicle_type && sr.season_type = s.season_type && sr.status = 'ACTIVE') where s.holder_id = ${db.escape(user.user_id)} and s.status in ('NEW', 'ACTIVE') and s.is_latest = true and ${prepare_where(where, db)} ${order} ${limitation} ${offsetion}`
        const season = await db.query(sql)
        // let result = []
        // season.forEach(s => {
...
const list_season_for_admin = async data => {
    try {
        ...
        const sql = `select s.*, c.carpark_name, c.carpark_code, c.address, c.postal_code, c.public_policy, c.billing_method, c.allow_giro, c.allow_auto_renew, c.status as carpark_status, c.remarks, c.giro_form_id, item.invoice_item_id, item.unit_price, item.quantity, item.amount, item.invoice_id, item.description, item.*, f.file_type, f.name as filename, f.mimetype, f.size, sr.season_rate_id, sr.rate, sr.status as rate_status, sr.remarks as rate_remarks from season s left join carpark c using(carpark_id) left join invoice_item item on item.invoice_item_id = (select invoice_item.invoice_item_id from invoice_item where s.season_id = invoice_item.season_id order by invoice_item.invoice_item_id desc limit 1) left join file f using (file_id) left join season_rate sr on (sr.carpark_id = c.carpark_id && sr.vehicle_type = s.vehicle_type && sr.season_type = s.season_type && sr.status = 'ACTIVE') where ${prepare_where(where, db)} ${order} ${limitation} ${offsetion}`
        const season = await db.query(sql)
        return season
    } catch (err) {
        return err
